  ...
  public:
    ...
    private:
      ...
      ubyte* Match_Output3(uint len, uint distance, ubyte* output) {
       ...
       ubyte* Match_Output4(uint len, uint distance, ubyte* output) { //Window_Num=n
             --distance;
        uint index=0;
          if (distance < MAX_L2_Length) {
          if (len < 7) {
       ...
       return &output[index];
          ...
          ubyte* Match_Output5(uint len, uint distance, ubyte* output) { //Window_Num=2
        --distance;
        uint index=0;
        if (distance < MAX_L2_Length) {
          if (len < 6) {
            output[index++] = (len << 5) + (distance >> 8);
            output[index++] = (distance & 0xff);
          } else if(len<6+255){ //not 1111 1111
            output[index++] = (7 << 5) + (distance >> 8);   //110
            // for (len -= 6; len >= 0xff; len -= 0xff) output[index++] = 0xff;
            len-=6;
            output[index++] = len;
            output[index++] = (distance & 0xff);
          }else if(len<6+255+(1<<16)){
            uint flag=6;
            len-=6;
            len-=255;
            output[index++] = (flag << 5) + (distance >> 8);
            output[index++]=(len>>8);  //D15-D0
            output[index++]=(len&0xff);
            output[index++] = (distance & 0xff);

          }else if(len>=6+255+(1<<16)){
            output[index++] = (7 << 5) + (distance >> 8);   //110
            output[index++] = 0xff; //1111 1111
            for (len -= 6; len >= 0xff; len -= 0xff) output[index++] = 0xff;
            output[index++] = len;
            output[index++] = (distance & 0xff);
          }
          ...
          /* still far away... */
          if (len < 6) {//Level 2 Extended Windows* Short match     //Direct Match 7->6
            distance -= MAX_L2_Length;
            output[index++] = (len << 5) + 0x1f;  
            output[index++] = 0xff;
            output[index++] = distance >> 8;
            output[index++] = distance & 0xff;
          } else if(len<6+255){//Level 2 Extended Windows* Long match
            distance -= MAX_L2_Length;
            output[index++] = (7 << 5) + 0x1f;  //110
            // for (len -= 6; len >= 0xff; len -= 0xff) output[index++] = 0xff;
            len-=6;
            output[index++] = len;
            output[index++] = 0xff;
            ...
            if (len < 6) {//Level 3 Extended Windows* Short match
             distance -= MAX_L2_Length;
             distance -= 65535;
            output[index++] = (len << 5) + 0x1f;
            output[index++] = 0xff;
            ...
            output[index++] = distance >> 8;
            output[index++] = distance & 0xff;
          } else if(len<6+255){ //Level 3 Extended Windows* Long match //not 1111 1111
            distance -= MAX_L2_Length;
            distance -= 65535;
            output[index++] = (7 << 5) + (distance >> 8);   //110
            // for (len -= 6; len >= 0xff; len -= 0xff) output[index++] = 0xff;
    ...
    void level4() { //Window_Num
      ...
      while (input < input_limit)[[likely]] {
          ...
          match_len = match_cmp(ref + 3, input + 3, input_bound);
          output = Match_Output5( match_len, distance, output);
