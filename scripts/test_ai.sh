...
jq -r '.[].name' ./sample-models/model-index.json | while read i; do
  ...
  encoded_file_path="./test/encoded/$i/"
  ...
  if [[ -f $input_file_path ]]; then
    # Run encoder
    python3 encode.py -i $input_file_path -o $encoded_file_path > /dev/null 2>&1

    # Calculate compression ratio
    if [[ -f $encoded_file_path ]] || [[ -d $encoded_file_path ]]; then
      ...
      RATIO=$(echo | awk "{print 1 - ($encoded_file_size / $input_file_size)}")
    ...
    # Run decoder
    { time python3 decode.py -i $encoded_file_path -o $decoded_file_path > /dev/null 2>&1 ; } 2> time.log
