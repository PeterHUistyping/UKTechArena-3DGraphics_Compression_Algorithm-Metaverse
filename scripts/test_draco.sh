...
jq -r '.[].name' ./sample-models/model-index.json | while read i; do
  ...
  encoded_file_path="./test/encoded/$i/$i.drc"
  ...
  if [[ -f $input_file_path ]]; then
    ...
    # Run decoder
    { time ./decoder -i $encoded_file_path -o $decoded_file_path > /dev/null 2>&1 ; } 2> test/time.log

    # Calculate decompression time
    if [[ -f $decoded_file_path ]]; then
      TIME=$(awk '/user|sys/ {split($2,a,/m|s/); sum+=(a[1] * 60000) + (a[2] * 1000)} END { print sum }' test/time.log)
